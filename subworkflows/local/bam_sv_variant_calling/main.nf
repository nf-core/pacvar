...
include { BCFTOOLS_INDEX as BCFTOOLS_INDEX_SV    } from '../../../modules/nf-core/bcftools/index/main'
...
workflow BAM_SV_VARIANT_CALLING {
    ...
    BCFTOOLS_INDEX_SV(TABIX_BGZIP.out.output)

    vcf_ch = TABIX_BGZIP.out.output.join(BCFTOOLS_INDEX_SV.out.csi)

    ch_versions = ch_versions.mix(PBSV_DISCOVER.out.versions)
    ch_versions = ch_versions.mix(PBSV_CALL.out.versions)
    ch_versions = ch_versions.mix(TABIX_BGZIP.out.versions)
    ch_versions = ch_versions.mix(BCFTOOLS_INDEX_SV.out.versions)
